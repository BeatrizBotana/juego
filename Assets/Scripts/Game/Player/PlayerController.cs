...
namespace Gamekit3D
{
    ...
    public class PlayerController : MonoBehaviour, IMessageReceiver
    {
        ...
        public float maxForwardSpeed = 8f;        // How fast Puppy can run.
        public float gravity = 20f;               // How fast Puppy accelerates downwards when airborne.
        public float jumpSpeed = 10f;             // How fast Puppy takes off when jumping.
        public float minTurnSpeed = 400f;         // How fast Puppy turns when moving at maximum speed.
        public float maxTurnSpeed = 1200f;        // How fast Puppy turns when stationary.
        public float idleTimeout = 5f;            // How long before Puppy starts considering random idles.
        public bool canAttack;                    // Whether or not Puppy can swing her staff.
        ...
        protected bool m_IsGrounded = true;            // Whether or not Puppy is currently standing on the ground.
        protected bool m_PreviouslyGrounded = true;    // Whether or not Puppy was standing on the ground last frame.
        protected bool m_ReadyToJump;                  // Whether or not the input state and Puppy are correct to allow jumping.
        protected float m_DesiredForwardSpeed;         // How fast Puppy aims be going along the ground based on input.
        protected float m_ForwardSpeed;                // How fast Puppy is currently going along the ground.
        protected float m_VerticalSpeed;               // How fast Puppy is currently moving up or down.
        protected PlayerInput m_Input;                 // Reference used to determine how Puppy should move.
        protected CharacterController m_CharCtrl;      // Reference used to actually move Puppy.
        protected Animator m_Animator;                 // Reference used to make decisions based on Puppy's current animation and to set parameters.
        protected Material m_CurrentWalkingSurface;    // Reference used to make decisions about audio.
        protected Quaternion m_TargetRotation;         // What rotation Puppy is aiming to have based on input.
        protected float m_AngleDiff;                   // Angle in degrees between Puppy's current rotation and her target rotation.
        protected Collider[] m_OverlapResult = new Collider[8];    // Used to cache colliders that are near Puppy.
        protected bool m_InAttack;                     // Whether Puppy is currently in the middle of a melee attack.
        protected bool m_InCombo;                      // Whether Puppy is currently in the middle of her melee combo.
        ...
        protected Checkpoint m_CurrentCheckpoint;      // Reference used to reset Puppy to the correct position on respawn.
        protected bool m_Respawning;                   // Whether Puppy is currently respawning.
        protected float m_IdleTimer;                   // Used to count up to Puppy considering a random idle.

        // These constants are used to ensure Puppy moves and behaves properly.
        ...
        readonly int m_HashPuppyCombo1 = Animator.StringToHash("PuppyCombo1");
        readonly int m_HashPuppyDeath = Animator.StringToHash("PuppyDeath");
        ...
        {
            bool equipped = m_NextStateInfo.shortNameHash == m_HashPuppyCombo1 || m_CurrentStateInfo.shortNameHash == m_HashPuppyCombo1;
        ...
        void CalculateForwardMovement()
        {
            ...
            if (moveInput.sqrMagnitude > 1f)
                moveInput.Normalize();
            Console.WriteLine("Entro:"+ moveInput);
        ...
        {
            // If jump is not currently held and Puppy is on the ground then she is ready to jump.
            if (!m_Input.JumpInput && m_IsGrounded)
                m_ReadyToJump = true;

            if (m_IsGrounded)
            {
                // When grounded we apply a slight negative vertical speed to make Puppy "stick" to the ground.
                m_VerticalSpeed = -gravity * k_StickingGravityProportion;

                // If jump is held, Puppy is ready to jump and not currently in the middle of a melee combo...
            ...
            {
                // If Puppy is airborne, the jump button is not held and Puppy is currently moving upwards...
                if (!m_Input.JumpInput && m_VerticalSpeed > 0.0f)
                {
                    // ... decrease Puppy's vertical speed.
                ...
                // If Puppy is airborne, apply gravity.
                m_VerticalSpeed -= gravity * Time.deltaTime;
            }
        }

        // Called each physics step to set the rotation Puppy is aiming to have.
        void SetTargetRotation()
        {
            ...
            // The desired forward direction of Puppy.
            ...
            if (m_InAttack)
            {
                ...
                for (int i = 0; i < count; ++i)
                {
                    ...
                    // This will be larger the closer to Puppy's desired direction the direction to the enemy is.
        ...
        // Called each physics step to help determine whether Puppy can turn under player input.
        ...
        // Called each physics step after SetTargetRotation if there is move input and Puppy is in the correct animator state according to IsOrientationUpdated.
        ...
        void PlayAudio()
        {
            ...
            if (m_CurrentStateInfo.shortNameHash == m_HashPuppyDeath && m_PreviousCurrentStateInfo.shortNameHash != m_HashPuppyDeath)
            {
                emoteDeathPlayer.PlayRandomClip();
            }

            if (m_CurrentStateInfo.shortNameHash == m_HashPuppyCombo1 && m_PreviousCurrentStateInfo.shortNameHash != m_HashPuppyCombo1)
            {
                emoteAttackPlayer.PlayRandomClip();
            }
        }

        // Called each physics step to count up to the point where Puppy considers a random idle.
        void TimeoutToIdle()
        {
        ...
        void OnAnimatorMove()
        {
            ...
            // Send whether or not Puppy is on the ground to the animator.
            m_Animator.SetBool(m_HashGrounded, m_IsGrounded);
        }

        // This is called by an animation event when Puppy swings her staff.
        ...
        // This is called by an animation event when Puppy finishes swinging her staff.
        ...
        // This is called by Checkpoints to make sure Puppy respawns correctly.
        ...
        {
            // Wait for the animator to be transitioning from the PuppyDeath state.
            while (m_CurrentStateInfo.shortNameHash != m_HashPuppyDeath || !m_IsAnimatorTransitioning)
            {
                yield return null;
            }
            ...
            // Enable spawning.
            PuppySpawn spawn = GetComponentInChildren<PuppySpawn>();
            spawn.enabled = true;

            // If there is a checkpoint, move Puppy to it.
        ...
        // Called by a state machine behaviour on Puppy's animator controller.
        ...
        // Called by Puppy's Damageable when she is hurt.
